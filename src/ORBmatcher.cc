...
int ORBmatcher::TwoFrameMatches(Frame &F1, Frame &F2, vector<int> &vnMatches12, int windowSize)
{
    int nmatches=0;
    vnMatches12 = vector<int>(F1.mvKeysUn.size(),-1);
...
vector< vector<int> > ORBmatcher::MatchByObjectAll(Frame &F1, Frame &F2, int windowSize)
{
    int nmatches=0;
    vector <vector<int> > RetVec;
    // cout << "Total number of objects: " << F1.mvvObjKeys.size() << endl;

    for(size_t obj=0; obj < F1.mvvObjKeys.size(); obj++)
    {
        // cout << "Looping over object " << obj << " with " << F1.mvvObjKeys.size() << " keypoints" << endl;
        vector<size_t> CurrObjKeys = F1.mvvObjKeys[obj];
        // vector<int> CurrObjMatches(CurrObjKeys.size(),-1);
        vector<int> CurrObjMatches (CurrObjKeys.size(),-1);

        vector<int> vMatchedDistance(F2.mvKeysUn.size(),INT_MAX);
        vector<int> vnMatches21(F2.mvKeysUn.size(),-1);

        for(size_t kp=0; kp < CurrObjKeys.size(); kp++)
        {
            size_t i1 = CurrObjKeys[kp];
            cv::KeyPoint kp1 = F1.mvKeysUn[i1];
            int level1 = kp1.octave;
            if(level1>0)
                continue;
            ...
            // cout << "Loop 2 with " << vIndices2.size() << " features in area" << endl;
            ...
            {
                size_t i2 = *vit2;
                // cout << "Loop 3 at index " << i2 << endl;

                cv::Mat d2 = F2.mDescriptors.row(i2);
                int ObjLabel2 = F2.mvKeyObjLabels[i2];
            ...
            if(bestDist<=TH_LOW)
            {
                if(bestDist<(float)bestDist2*mfNNratio)
                {
                    if(vnMatches21[bestIdx2]>=0)
                    {
                        // cout << "Overladed match, discount" << endl;
                        // CurrObjMatches[vnMatches21[bestIdx2]]=-1; // THIS LINE IS BROKEN!!
                        nmatches--;
                    }
                    // cout << "Assigning match to keypoint " << i1 << ", curent index " << kp << endl;
                    CurrObjMatches[kp]=bestObjLabel;
                    // CurrObjMatches[kp]=bestIdx2;
                    vnMatches21[bestIdx2]=i1;
                    vMatchedDistance[bestIdx2]=bestDist;
                    nmatches++;
        ...
        RetVec.push_back(CurrObjMatches);
    }

    return RetVec;
}

int ORBmatcher::MatchByObject(Frame &F1, Frame &F2, vector<int> &ObjMatches, int windowSize, int ObjId)
{
    int nmatches=0;

    vector<size_t> CurrObjKeys = F1.mvvObjKeys[ObjId];
    // vector<int> CurrObjMatches(CurrObjKeys.size(),-1);
    ObjMatches = vector<int>(CurrObjKeys.size(),-1);
    ...
    for(size_t kp=0; kp < CurrObjKeys.size(); kp++)
    {
        ...
        if(bestDist<=TH_LOW)
        {
            if(bestDist<(float)bestDist2*mfNNratio)
            {
                if(vnMatches21[bestIdx2]>=0)
                {
                    // ObjMatches[vnMatches21[bestIdx2]]=-1; // THIS LINE IS BROKEN!!
                    nmatches--;
                }
                ObjMatches[kp]=bestObjLabel;
                vnMatches21[bestIdx2]=i1;
                vMatchedDistance[bestIdx2]=bestDist;
                nmatches++;
